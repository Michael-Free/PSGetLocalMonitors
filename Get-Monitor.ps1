function Get-LocalMonitors {
    ...
    foreach ($Monitor in $Monitors) {
        if ($null -eq $Monitor.UserFriendlyName) {
            Write-Host "UserFriendlyName is null"
            $Mon_Model = "Unknown"
        } else {
            Write-Host "UserFriendlyName is not null"
            $Mon_Model = [System.Text.Encoding]::ASCII.GetString($Monitor.UserFriendlyName)
            Write-Host "Raw decoded string: '$Mon_Model'"
        }
        #if ([System.Text.Encoding]::ASCII.GetString($Monitor.UserFriendlyName) -ne $null) {

        #    $Mon_Model = ([System.Text.Encoding]::ASCII.GetString($Monitor.UserFriendlyName)).Replace([char]0.ToString(), "")
